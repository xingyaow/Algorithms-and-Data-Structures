...
// empty buffers into a music string
   ...
   public static Cons varsb (Object expr, Cons result){
    ...
    public static Cons solve(Cons e, String v) {
       if(lhs(e).equals(v)) return e;
       if(rhs(e).equals(v)) return list(op(e), rhs(e), lhs(e));
       if(!consp(rhs(e))) return null;
       Object tranItem = null;
       Cons solved = null;
       for(Cons temp = algpats; temp != null; temp = rest(temp)){
	  tranItem = transform((Cons)first(temp), e);
	  if(tranItem != null) solved = solve((Cons)tranItem, v);
    ...
    public static int conshash(Object e) {
       if(e == null) return 0;
       return (((String)first((Cons)e)).hashCode()*17)^(conshash(rest((Cons)e))*127);
    }

    // total time to execute an action
    public static int totaltime(Cons action) {
       // int totalTime = 0;
       // for(Cons temp = action; temp != null; temp = rest(temp)){
       // 	  if(first(action).equals("repeat")); //totalTime += (int)second(temp)*totaltime((Cons)third(action));
       // 	  else if(first(action).equals("seq"));
       // 	  else if(first(action).equals("sync"));
       // 	  else if(first(action).equals("piano"));
	  
       //    else if(first(action) != null) totalTime += (Integer)second(action);
       // }
       // return totalTime;
       if(action == null) return 0;
       if(first(action).equals("repeat")) return (int)second(temp)*totaltime((Cons)third(action));
       if(first(action).equals("seq"))return totaltime((Cons)rest(action));
       if(first(action).equals("sync")) return totaltime;
	...
	if ( command.equals("kaboom")) {
	   emit(5, time, (int)(Integer)second(act));
	   return; }
	//I'm not really sure what I'm supposed to do here, because I don't know if I need to queue up the commands to play, or to just play them. This was kind of confusing, and I couldn't get Jfugue installed correctly to test it.
